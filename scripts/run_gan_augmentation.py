...
def run_gan_augmentation(force_reprocess_data=False, force_retrain_gans=False):
    ...
    features_for_model = gan_data_handler.get_features_for_model()
    target_feature = gan_data_handler.get_target_feature()
    scaler_X, scaler_y = gan_data_handler.get_scalers()

    # Data dimension for GAN: number of input features + 1 (for log_Id)
    data_dim = len(features_for_model) + 1
    ...
    augmented_dfs_by_region = {}  # To store synthetic data generated by each GAN

    # Determine majority class size for balancing
    ...
    # Calculate the target size for all regions
    # This is the maximum existing region size plus the additional_majority_samples from config
    ...
    for region_name, df_region in segregated_data_dfs.items():
        ...
        if settings.get(
                'run_flags.skip_gan_training_if_exists') and gan_trainer.load_models() and not force_retrain_gans:
