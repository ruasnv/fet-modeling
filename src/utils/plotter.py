...
class Plotter:
    ...
    def prepare_model_input_and_predict(
        ...
        Args:
            ...
            x_range_dense (np.ndarray): A dense array of values for the sweep axis (Vds or Vgs).
            ...
            region_name (str): The name of the operating region (e.g., "Cut-off", "Linear", "Saturation")
                                or plot type for warning messages.
            fixed_val_for_synthetic (float): The fixed Vgs or Vds value for synthetic data generation.
            sweep_axis_col (str): The column name for the sweep axis ('vd' or 'vg').
        """
        model.eval()
...
#TODO: Debug needed "Expected type 'float', got 'ndarray' instead "
        if sweep_axis_col == 'vd':
            ...
            synthetic_data_dict['vg'] = x_range_dense
            synthetic_data_dict['vd'] = fixed_val_for_synthetic
        else:
    ...
    def id_vds_characteristics(
        ...
        for region, cases_list in cases_config_for_best_worst_plots.items():
            ...
            for case_dict in cases_list:
                ...
                # For these dynamically suggested cases, plot_type is always Id_Vds_fixed_Vgs
                plot_type = case_dict.get('plot_type', 'Id_Vds_fixed_Vgs')  # Default to Id_Vds
