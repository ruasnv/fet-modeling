...
# Correct Imports based on our clean structure
from src.data.loader import DataLoader
from src.physics.equations import calculate_dynamic_vth, classify_region
from src.core.config import settings

class DataPreprocessor:
    def __init__(self, raw_data_path=None, processed_data_dir=None):
        """
        Initializes the DataPreprocessor.
        
        Args:
            raw_data_path (str/Path, optional): Override default raw data path.
            processed_data_dir (str/Path, optional): Override default output dir.
        """
        # FLEXIBILITY: Use arguments if provided, otherwise fall back to config
        self.raw_data_path = raw_data_path if raw_data_path else settings.get('paths.raw_data_path')
        self.processed_data_dir = processed_data_dir if processed_data_dir else settings.get('paths.processed_data_dir')
        
        # Ensure paths are Path objects
        if self.raw_data_path: self.raw_data_path = Path(self.raw_data_path)
        if self.processed_data_dir: self.processed_data_dir = Path(self.processed_data_dir)
    ...
    def _apply_filters(self, df):
        ...
        temp_filter = settings.get('filtering.temperature_filter')
        if temp_filter is not None:
            filtered_df = filtered_df[np.isclose(filtered_df['temp'], temp_filter)].copy()
            if settings.get('global_settings.debug_mode', False):
                print(f"DEBUG: After Temp = {temp_filter}°C filter: {len(filtered_df)} rows")
